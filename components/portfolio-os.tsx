...
import { ArrowUpRight, BriefcaseBusiness, Code2, ContactRound, FileText, FolderKanban, GitBranch, Grid2X2, Mail, MessageCircle, Minimize2, Monitor, Terminal, UserRound, X } from 'lucide-react';
import Image from 'next/image';
import Link from 'next/link';
import { useEffect, useMemo, useRef, useState } from 'react';
import { education, experience, profile, profileHighlights, projects, stack } from '../data';

type AppId = 'about' | 'work' | 'experience' | 'contact' | 'resume';
type OpenWindow = {
  id: Exclude<AppId, 'resume'>;
  x: number;
  y: number;
  z: number;
  width: number;
...
function WindowFrame({
  ...
  return (
    <section
      className="os-window"
      aria-label={title}
      style={{ transform: `translate(${windowState.x}px, ${windowState.y}px)`, zIndex: windowState.z, width: windowState.width, height: windowState.height }}
      onPointerDown={onFocus}
    >
      <header className="window-bar" onMouseDown={startDrag}>
        <div className="window-dots">
          <button className="dot close" onClick={onClose} onMouseDown={(event) => event.stopPropagation()} aria-label="Close window">
            <X size={11} />
          </button>
          <button className="dot minimize" onClick={onMinimize} onMouseDown={(event) => event.stopPropagation()} aria-label={`Minimize ${title}`}>
            <Minimize2 size={9} />
          </button>
    ...
    <div className="about-app">
      <div className="identity-card">
        <div className="avatar-wrap">
          <Image src="/pic.webp" alt="Mehfooz-ur-Rehman" width={150} height={150} priority />
        </div>
        <div>
  ...
  return (
    <div className="work-app">
      <div className="app-intro">
        <p className="app-kicker">PROJECT EXPLORER / {String(projects.length).padStart(2, '0')}</p>
        <h1>Shipped, not shelved.</h1>
      </div>
        ...
        {projects.map((project, index) => (
          <article className="project-file" key={project.slug}>
            ...
            <div className="project-actions">
              ...
              {project.url && (
                <a href={project.url} target="_blank" rel="noreferrer" aria-label={`Visit ${project.title}`}>
...
function ExperienceApp() {
  return (
    <div className="experience-app">
      <div className="app-intro">
        <p className="app-kicker">TIMELINE / CAREER LOG</p>
        <h1>Systems, products, outcomes.</h1>
      </div>
      ...
      <div className="education-row">
        {education.map((item) => (
          <div key={item.school}>
            <p className="app-kicker">EDUCATION</p>
            <h2>{item.school}</h2>
            <p>
...
function ContactApp() {
  const lines = useMemo(() => ['mehf@portfolio:~$ init collaboration', '> connection ready', '> product thinking: online', '> full-stack execution: online'], []);
  return (
    <div className="contact-app">
      ...
      <div className="contact-grid">
        ...
        <a href={profile.github} target="_blank" rel="noreferrer">
          <GitBranch size={20} />
          <span>GitHub</span>
          <strong>See the source</strong>
        </a>
      </div>
      <p className="contact-note">Send the workflow, product idea, or stuck project. I’ll help turn it into something people can use.</p>
    </div>
  );
}
...
export function PortfolioOS() {
  ...
  const focusApp = (id: OpenWindow['id']) =>
    setWindows((current) => {
      const topZ = Math.max(0, ...current.map((item) => item.z)) + 1;
  ...
  const maximizeApp = (id: OpenWindow['id'], bounds: DOMRect) =>
    setWindows((current) =>
      current.map((item) => {
  ...
  return (
    <main className={`os-shell ${booted ? 'is-booted' : ''}`}>
      <div className="grain" />
      <header className="menubar">
        <div>
          <Monitor size={14} /> <b>MEHFOOZ OS</b>
          <span>Finder</span>
          <span>Projects</span>
          <span>View</span>
        </div>
        <div>
          <span className="live-dot" /> BUILDING THE NEXT THING <time>2026</time>
        </div>
      </header>
      <div className="desktop">
        <section className="desktop-intro">
          <p className="boot-label">WELCOME TO</p>
          <h1>
            Mehfooz
            <br />
            <em>OS</em>
          </h1>
          <p>A working desktop for a full-stack product developer.</p>
        </section>
        <nav className="desktop-icons" aria-label="Portfolio applications">
          {apps.map((app) => {
            const Icon = app.icon;
            return (
              <button className={windows.some((item) => item.id === app.id) ? 'selected' : ''} onClick={() => openApp(app.id)} key={app.id}>
                <span>
                  <Icon size={25} />
                </span>
                {app.label}
        ...
        <div className="workspace">
          {windows
            ...
            .map((windowState) => (
              <WindowFrame
                key={windowState.id}
                title={apps.find((app) => app.id === windowState.id)!.label}
                windowState={windowState}
                onClose={() => closeApp(windowState.id)}
                onFocus={() => focusApp(windowState.id)}
                onMove={(x, y) => moveApp(windowState.id, x, y)}
                onResize={(width, height) => resizeApp(windowState.id, width, height)}
                onMinimize={() => minimizeApp(windowState.id)}
                onMaximize={(bounds) => maximizeApp(windowState.id, bounds)}
              >
      ...
      <footer className="dock">
        {apps.slice(0, 4).map((app) => {
          ...
          return (
            <button className={`${state ? 'active' : ''} ${state?.minimized ? 'minimized' : ''}`} onClick={() => openApp(app.id)} aria-label={app.label} data-label={app.label} key={app.id}>
              <Icon size={21} />
            </button>
          );
        })}
        <span />
        <a href={profile.linkedin} target="_blank" rel="noreferrer" aria-label="LinkedIn" data-label="LinkedIn">
          <ContactRound size={20} />
        </a>
        <button onClick={openAllApps} aria-label="Open all portfolio applications" data-label="Open all apps">
          <Grid2X2 size={19} />
        </button>
      </footer>
    </main>
